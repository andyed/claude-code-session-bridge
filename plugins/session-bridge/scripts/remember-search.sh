#!/usr/bin/env bash
# scripts/remember-search.sh — Search conversation history via logs and transcripts.
...
if [ "$SEARCH_TRANSCRIPTS" = true ]; then
  ...
  for transcript in "$TRANSCRIPTS_DIR"/*/*.jsonl; do
    ...
    local session_file=$(basename "$transcript")
    local session_id="${session_file%.jsonl}"
    local project_dir=$(basename "$(dirname "$transcript")")
